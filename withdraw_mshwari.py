...
class AmountWithdraw():
    def __init__(self, root):
        ...
        # Withdraw Cash On Mshwari on Top
        Label(self.root1, text="M-Shwari", font=("Times New Roman", 16), background="white").place(x=60, y=68)
    ...
    def ok(self):
        if self.pin_var.get() ==  "":
            ...
            if int(self.pin_var.get()) == self.actual_pin:
                ...
                time.sleep(2)
                accept = messagebox.askyesno(title="", message=f"    Withdraw from M-Shwari \n\nWithdraw from M-Shwari kSH {self.amount}", parent=self.root1)
                if accept == False:
                    self.root1.destroy()
                else:
                    time.sleep(2)
                    mess = messagebox.askyesno(title="", message=f"    Withdraw from M-Shwari \n\nSent\nWait for M-PESA to reply", parent=self.root1)
                    if mess == False:
                        self.root1.quit
                    else:
                        time.sleep(2)
                        ...
                        self.response = f"""RH{random} Confirmed, Ksh{self.amount} transferred from M-Shwari account on {self.now}  
