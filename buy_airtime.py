...
import random as re
import string
from airtime_other_number import AirtimeOthers
...
class BuyAirtime():
    def __init__(self, root):
        ...
        # Buy Airtime Name on Top
        Label(self.root, text="Buy Airtime", font=("Times New Roman", 16), background="white").place(x=60, y=68)
        ...
        buy_airtime_btn.place(x=0, y=130)



        # ==================================Functions=========================================
    def buy_airtime(self):
        OptionsBuyAirtime(Toplevel(self.root))

    def back(self):
        self.root.destroy()

# Class For Option Mode Of buying Airtime
class OptionsBuyAirtime():
    def __init__(self, root):
        self.root = root
        self.root.title("")
        self.root.geometry("350x550+500+50")
        ...
        # ===============================Main Body=======================================
        # My Phone Button
        my_phone_btn = CTkButton(self.root,command=self.my_phone, text="My Phone",text_color="black",width=360,border_width=0,corner_radius=0,
         fg_color="white", bg_color="white", font=("Bookman old Style", 17), hover_color="light gray", anchor=W, height=42)
        my_phone_btn.place(x=0, y=130)

        # Other Phone Button
        other_phone_btn = CTkButton(self.root,command=self.other_phone, text="Other Phone",text_color="black",width=360,border_width=0,corner_radius=0,
         fg_color="white", bg_color="white", font=("Bookman old Style", 17), hover_color="light gray", anchor=W, height=42)
        other_phone_btn.place(x=0, y=180)



        # ==================================Functions=========================================
    def my_phone(self):
        Amount(Toplevel(self.root))
    def other_phone(self):
        AirtimeOthers(Toplevel(self.root))
    def back(self):
        self.root.destroy()
...
class Amount():
    def __init__(self, root):
        ...
        Label(self.root1, text="Buy Airtime", font=("Times New Roman", 16), background="white").place(x=60, y=68)
    ...
    def ok(self):
        if self.pin_var.get() ==  "":
            ...
            if int(self.pin_var.get()) == self.actual_pin:
                ...
                time.sleep(2)
                accept = messagebox.askyesno(title="", message=f"    My Phone \n\n Buy Airtime for My Phone kSH {self.amount}", parent=self.root1)
                if accept == False:
                    self.root1.destroy()
                else:
                    time.sleep(2)
                    mess = messagebox.askyesno(title="", message=f"    My Phone \n\nSent\nWait for M-PESA to reply", parent=self.root1)
                    if mess == False:
                        self.root1.quit
                    else:
                        time.sleep(2)
                        ...
                        self.response = f"""RH{random} Confirmed, You bought Ksh{self.amount} of airtime on {self.now}  
