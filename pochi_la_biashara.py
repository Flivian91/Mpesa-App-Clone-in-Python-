...
class PochiBiashara():
    def __init__(self, root):
        ...
        # Pochi la biashara name on Top
        Label(self.root1, text="Pochi la Biashara", font=("Times New Roman", 16), background="white").place(x=60, y=68)
        
        # ================================end upper==========================================

        # Label to show user what to enter
        Label(self.root1, text="Enter Phone No.", font=("Times New Roman", 16), background="white").place(x=80, y=148)

        # Entry Field for Phone number

        self.pochi_biashara_var = StringVar() #String variable to store Phone Number Entered by the user

        pochi_biashara = CTkEntry(self.root1, width=300, height=40,font=("Bookman Old Style", 20),textvariable=self.pochi_biashara_var, 
        border_width=0, fg_color="white", corner_radius=0, text_color="black")
        pochi_biashara.place(x=20, y=188)
        pochi_biashara.focus()
        # Label Indicator
        ...
        Label(self.root1, text="Digits (0-9,*,#,+) 10 - 13", font=("Times New Roman", 12), background="white").place(x=20, y=234)
    ...
    def ok(self):
        if self.pochi_biashara_var.get() ==  "":
            self.indicator_label.config(background="red")
        elif len(self.pochi_biashara_var.get()) > 13:
            self.indicator_label.config(background="red")
        elif len(self.pochi_biashara_var.get()) < 10:
            self.indicator_label.config(background="red")
        elif self.pochi_biashara_var.get().isdecimal() == False:
            self.indicator_label.config(background="red")
        else:
            pochi_biashara = self.pochi_biashara_var.get()
            Amount(Toplevel(self.root1),pochi_biashara)
...
class Amount():
    def __init__(self, root, pochi_biashara):
        self.root1 = root
        self.root1.title("")
        self.root1.geometry("350x550+500+50")
        self.root1.overrideredirect(1)
        self.root1.config(background="white")
        self.pochi_biashara = pochi_biashara
    ...
    def ok(self):
        if self.amount_var.get() ==  "":
            ...
            pochi_biashara = self.pochi_biashara
            amount = self.amount_var.get()
            Pin(Toplevel(self.root1), pochi_biashara, amount)
...
class Pin():
    def __init__(self, root, pochi_biashara, amount):
        self.root1 = root
        self.root1.title("")
        self.root1.geometry("350x550+500+50")
        self.root1.overrideredirect(1)
    ...
    def ok(self):
        if self.pin_var.get() ==  "":
            ...
            if int(self.pin_var.get()) == self.actual_pin:
                ...
                time.sleep(2)
                accept = messagebox.askyesno(title="", message=f"    Send Money to Pochi la Biashara \n\n Send Money to {str(self.pochi_biashara)} kSH {self.amount}", parent=self.root1)
                if accept == False:
                    self.root1.destroy()
                else:
                    time.sleep(2)
