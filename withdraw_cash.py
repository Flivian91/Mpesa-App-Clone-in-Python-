...
from agent_withdraw import Agent
from atm_withdraw import ATM
...
class MainWithdraw():
    def __init__(self, root):
        ...
        Label(self.root, text="Withdraw Cash", font=("Times New Roman", 16), background="white").place(x=60, y=68)
        
        # ================================end upper==========================================

        # ===============================Main Body=======================================
        # Withdraw From Agent Button
        from_agent_btn = CTkButton(self.root,command=self.from_agent, text="From Agent",text_color="black",width=360,border_width=0,corner_radius=0,
         fg_color="white", bg_color="white", font=("Bookman old Style", 17), hover_color="light gray", anchor=W, height=42)
        from_agent_btn.place(x=0, y=130)

        # Withdraw From ATM Button
        from_atm_btn = CTkButton(self.root,command=self.from_atm, text="From ATM",text_color="black",width=360,border_width=0,corner_radius=0,
         fg_color="white", bg_color="white", font=("Bookman old Style", 17), hover_color="light gray", anchor=W, height=42)
        from_atm_btn.place(x=0, y=180)


        # ==================================Functions=========================================
    def from_agent(self):
        Agent(Toplevel(self.root))

    def from_atm(self):
        ATM(Toplevel(self.root))
