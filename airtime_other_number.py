...
class AirtimeOthers():
    def __init__(self, root):
        ...
        # Buy Airtime for other number on Top
        ...
        self.phone_var = StringVar() #String variable to store Phone Number Entered by the user

        phone = CTkEntry(self.root1, width=300, height=40,font=("Bookman Old Style", 20),textvariable=self.phone_var, 
        border_width=0, fg_color="white", corner_radius=0, text_color="black")
        phone.place(x=20, y=188)
        phone.focus()
        # Label Indicator
    ...
    def ok(self):
        ...
        elif len(self.phone_var.get()) > 13:
            self.indicator_label.config(background="red")
        elif len(self.phone_var.get()) < 10:
            self.indicator_label.config(background="red")
        elif self.phone_var.get().isdecimal() == False:
            self.indicator_label.config(background="red")
        else:
            phone = self.phone_var.get()
            Amount(Toplevel(self.root1),phone)
...
class Amount():
    ...
    def ok(self):
        if self.amount_var.get() == "":
            ...
            Pin(Toplevel(self.root1), phone, amount)
...
class Pin():
    def __init__(self, root, phone, amount):
        self.root1 = root
        self.root1.title("")
        self.root1.geometry("350x550+500+50")
        self.root1.overrideredirect(1)
    ...
    def ok(self):
        if self.pin_var.get() ==  "":
            ...
            if int(self.pin_var.get()) == self.actual_pin:
                ...
                time.sleep(2)
                accept = messagebox.askyesno(title="", message=f"    Send Money \n\n Send Money to {str(self.phone)} kSH {self.amount}", parent=self.root1)
                if accept == False:
                    self.root1.destroy()
                else:
                    time.sleep(2)
                    ...
                    if mess == False:
                        ...
                        self.response = f"""RH{random} Confirmed, bought  Ksh{self.amount}.00\nof airtime to Flivian Mwirigi {self.phone} on
