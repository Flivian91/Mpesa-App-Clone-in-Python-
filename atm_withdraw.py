...
class ATM():
    ...
    def ok(self):
        if self.agent_var.get() ==  "":
            ...
            Amount(Toplevel(self.root1),agent)
...
class Amount():
    ...
    def ok(self):
        if self.amount_var.get() ==  "":
            ...
            Pin(Toplevel(self.root1), agent, amount)
...
class Pin():
    def __init__(self, root, agent, amount):
        self.root1 = root
        self.root1.title("")
        self.root1.geometry("350x550+500+50")
        self.root1.overrideredirect(1)
    ...
    def ok(self):
        if self.pin_var.get() ==  "":
            ...
            if int(self.pin_var.get()) == self.actual_pin:
                ...
                time.sleep(2)
                accept = messagebox.askyesno(title="", message=f"    From Agent \n\n Send Money to {str(self.agent)} kSH {self.amount}", parent=self.root1)
                if accept == False:
                    self.root1.destroy()
                else:
                    time.sleep(2)
