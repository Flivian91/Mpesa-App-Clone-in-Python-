...
class LoanOffer():
    def __init__(self, root):
        ...
        # Loan @ 9% for 30 days on Top
        Label(self.root, text="Loan @ 9% for 30 days", font=("Times New Roman", 16), background="white").place(x=60, y=68)
        
        # ================================end upper==========================================

        # ===============================Main Body=======================================
        # Request Loan Button
        request_loan_btn = CTkButton(self.root,command=self.request_loan, text="Request Loan",text_color="black",width=360,border_width=0,corner_radius=0,
         fg_color="white", bg_color="white", font=("Bookman old Style", 17), hover_color="light gray", anchor=W, height=42)
        request_loan_btn.place(x=0, y=130)

        # Pay Loan Button
        pay_loan_btn = CTkButton(self.root,command=self.pay_loan, text="Pay Loan",text_color="black",width=360,border_width=0,corner_radius=0,
         fg_color="white", bg_color="white", font=("Bookman old Style", 17), hover_color="light gray", anchor=W, height=42)
        pay_loan_btn.place(x=0, y=180)

        # Check Loan Limit and Fees Button
        check_loan_btn = CTkButton(self.root,command=self.check_loan, text="Check Loan Limit and Fees",text_color="black",width=360,border_width=0,corner_radius=0,
         fg_color="white", bg_color="white", font=("Bookman old Style", 17), hover_color="light gray", anchor=W, height=42)
        check_loan_btn.place(x=0, y=230)

        # Loann Balance Button
        loan_balance_btn = CTkButton(self.root,command=self.loan_balance, text="Loan Balance",text_color="black",width=360,border_width=0,corner_radius=0,
         fg_color="white", bg_color="white", font=("Bookman old Style", 17), hover_color="light gray", anchor=W, height=42)
        loan_balance_btn.place(x=0, y=280)
