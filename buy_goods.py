...
class BuyGoods():
    def __init__(self, root):
        ...
        # Buys Goods Name on Top
        Label(self.root1, text="Buy Goods and Services", font=("Times New Roman", 16), background="white").place(x=60, y=68)
        
        # ================================end upper==========================================

        # Label to show user what to enter
        Label(self.root1, text="Enter Till No.", font=("Times New Roman", 16), background="white").place(x=80, y=148)

        # Entry Field for Phone number

        self.till_no_var = StringVar() #String variable to store Phone Number Entered by the user

        till_no = CTkEntry(self.root1, width=300, height=40,font=("Bookman Old Style", 20),textvariable=self.till_no_var, 
        border_width=0, fg_color="white", corner_radius=0, text_color="black")
        till_no.place(x=20, y=188)
        till_no.focus()
        # Label Indicator
        ...
        Label(self.root1, text="Digits (0-9,*,#,+) 4 - 10", font=("Times New Roman", 12), background="white").place(x=20, y=234)
    ...
    def ok(self):
        if self.till_no_var.get() ==  "":
            self.indicator_label.config(background="red")
        elif len(self.till_no_var.get()) > 10:
            self.indicator_label.config(background="red")
        elif len(self.till_no_var.get()) < 4:
            self.indicator_label.config(background="red")
        elif self.till_no_var.get().isdecimal() == False:
            self.indicator_label.config(background="red")
        else:
            till_no = self.till_no_var.get()
            Amount(Toplevel(self.root1),till_no)
...
class Amount():
    def __init__(self, root, till_no):
        self.root1 = root
        self.root1.title("")
        self.root1.geometry("350x550+500+50")
        self.root1.overrideredirect(1)
        ...
        # Buy Goods And services Name on Top
        Label(self.root1, text="Buy Goods and Services", font=("Times New Roman", 16), background="white").place(x=60, y=68)
    ...
    def ok(self):
        if self.amount_var.get() ==  "":
            ...
            till_no = self.till_no
            amount = self.amount_var.get()
            Pin(Toplevel(self.root1), till_no, amount)
...
class Pin():
    def __init__(self, root, till_no, amount):
        self.root1 = root
        self.root1.title("")
        self.root1.geometry("350x550+500+50")
        self.root1.overrideredirect(1)
    ...
    def ok(self):
        if self.pin_var.get() ==  "":
            ...
            if int(self.pin_var.get()) == self.actual_pin:
                ...
                time.sleep(2)
                accept = messagebox.askyesno(title="", message=f"    Buy Goods and Services \n\n Send Money to {str(self.till_no)} kSH {self.amount}", parent=self.root1)
                if accept == False:
                    self.root1.destroy()
                else:
                    time.sleep(2)
                    mess = messagebox.askyesno(title="", message=f"    Buy goods and Services \n\nSent\nWait for M-PESA to reply", parent=self.root1)
                    if mess == False:
                        self.root1.quit
                    else:
                        time.sleep(2)
                        ...
                        self.response = f"""RH{random} Confirmed, Ksh{self.amount}.00\n sent to Equity Paybill Account to Account {self.till_no} on
